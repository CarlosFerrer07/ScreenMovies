...
class Security extends Connection
{
    ...
    public function doLogin()
    {
        if (count($_POST) > 0) {
            ...
            $_SESSION["loggedIn"] = $this->checkUser($user, $_POST["password"]) ? $user["usuario"] : false; //checkUser comprueba si el usuario está correctamente identificado o no
            if ($_SESSION["loggedIn"]) {
                //si la variable de session loggedIn está establecida nos redirige a el inicio.php
